...
PRINT 'No of customers: ' + CAST(@customer_count AS VARCHAR(10));
...
drop trigger p_after_i;
DELIMITER //
CREATE TRIGGER p_after_i
...
-- trigger for updating product log
DELIMITER //
...
BEGIN
    ...
    IF order_count > 0 THEN
        ...
        SET MESSAGE_TEXT = 'Cannot delete product because there are existing orders referencing it';
    END IF;
END //
...
CREATE OR REPLACE VIEW active_customers_hyd AS
SELECT c.cid, c.cname, c.addr
FROM customer c
WHERE c.age > 20 AND c.addr LIKE '%Hyderabad%';
